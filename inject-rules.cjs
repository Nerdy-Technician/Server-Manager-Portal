...
let src = fs.readFileSync('index.tsx', 'utf8');
...
const MARKER = 'const SettingsDashboard: React.FC = () => {';
if (!src.includes(MARKER)) { console.error('MARKER NOT FOUND'); process.exit(1); }

src = src.replace(MARKER, component + MARKER);
fs.writeFileSync('index.tsx', src);
console.log('Done! StreamKillRulesPanel injected successfully.');
